...
try
{
    ...
    Log.Logger = new LoggerConfiguration()
        ...
        .Enrich.FromLogContext()
        .Enrich.WithProperty("Application", "EngagementService")
        .WriteTo.Console(new Serilog.Formatting.Compact.CompactJsonFormatter())
    ...
    builder.Services.AddDbContext<EngagementDbContext>(options => options.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection"), b => b.MigrationsAssembly("CondoNet.Engagement.Infrastructure")));
    ...
    app.UseSwagger();
    app.UseSwaggerUI(c => { c.SwaggerEndpoint("v1/swagger.json", "CondoNet Engagement API V1"); c.RoutePrefix = "swagger"; });
