...
{
    using global::CondoNet.Accounting.Core.Entities;
    using global::CondoNet.Shared;
    using global::CondoNet.Shared.Accounting.DTOs;
    using global::CondoNet.Shared.Accounting.Events;
    using global::CondoNet.Shared.Interfaces;
    using MassTransit;
            ...
            IHttpContextAccessor httpContextAccessor)
        {
            ...
            public async Task<Result<TransactionDetailResponse>> GetTransactionByIdAsync(Guid transactionId)
            {
                ...
                var details = new TransactionDetailResponse(
                    ...
                    tx.Entries.Select(e => new EntryDetailResponse(
                        e.Id, e.AccountId, e.Account.Code, e.Account.Name, e.Debit, e.Credit, e.Reference)).ToList()
                );
            ...
            public async Task<Result<bool>> VoidTransactionAsync(Guid transactionId)
            {
                ...
                try
                {
                    ...
                    {
                        entry.Account.CurrentBalance -= AccountingService.CalculateBalanceImpact(entry.Account.Type, entry.Debit, entry.Credit);
                    }
